...
| `user/loginByEmail`<br>Given a registered user's email and password, returns their authUserId value.        | POST        | **Body Parameters:**<br>`( email, password )`<br>**Return type if no error:**<br>`{ user }` | **400 Error** when any of:<br>· email entered does not belong to a user<br>· password is not correct
| `user/loginByUsername`<br>Given a registered user's email and password, returns their authUserId value.        | POST        | **Body Parameters:**<br>`( username, password )`<br>**Return type if no error:**<br>`{ user }` | **400 Error** when any of:<br>· entered does not belong to a user<br>· password is not correct
